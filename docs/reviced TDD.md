...
- │   ├── master_sheet.xlsx          # Master consolidation file

- │   ├── /division_templates        # 10 division Excel files
...
- │   ├── data_loader.py             # Excel → JSON extraction

- │   ├── data_validator.py          # Validation rules

- │   ├── image_processor.py         # Auto-crop, resize

- │   ├── chart_generator.py         # Matplotlib charts

- │   ├── html_builder.py            # NEW: Jinja2 HTML generation

- │   ├── pdf_generator.py           # NEW: WeasyPrint PDF generation
...
- │   ├── cover_page.html            # Cover page

- │   ├── section_scorecards.html    # Scorecards section

- │   ├── section_districts.html     # District performance

- │   ├── section_board.html         # Board of directors

- │   ├── section_events.html        # Events section

- │   ├── section_financials.html    # Financial tables
...
**File:** /templates/section_events.html
...
**File:** /templates/section_districts.html
...
- from config import BASE_DIR, FONTS
...
-         self.env.filters\['embed_image'\] = self.embed_image
...
-         sections.append(self.render_template('cover_page.html', data))

-         # Scorecards

-         sections.append(self.render_template('section_scorecards.html', data))

-         # District performance

-         sections.append(self.render_template('section_districts.html', data))

-         # Board of directors

-         sections.append(self.render_template('section_board.html', data))

-         # Events

-         sections.append(self.render_template('section_events.html', data))

-         # Financial tables

-         sections.append(self.render_template('section_financials.html', data))
...
-             font_path_regular=str(FONTS\['sinhala_regular'\]\['path'\]),

-             font_path_bold=str(FONTS\['sinhala_bold'\]\['path'\]),
...
- from config import OUTPUT_DIR, FONTS, BASE_DIR
...
-             additional_css = CSS(string=self.\_get_additional_css())
...
-     def generate_from_file(self, html_file_path, output_filename):
...
-         return self.generate_pdf(html_content, output_filename)
...
- from config import initialize_system, OUTPUT_DIR, LOGS_DIR
...
-         logging.FileHandler(LOGS_DIR / 'generation.log'),
...
-     data = load_master_sheet(args.month)
...
-             validation_report = LOGS_DIR / f"validation_report\_{args.month}.txt"
...
-         process_all_event_images(data\['events'\])
...
-     html_builder = HTMLReportBuilder(mode=args.mode)

-     html_content = html_builder.build_complete_html(data)
...
-     html_file = html_output_dir / f"Report\_{args.month}\_{args.mode.upper()}.html"
...
-     pdf_generator = PDFGenerator(mode=args.mode)

-     pdf_filename = f"Report\_{args.month}\_{args.mode.upper()}.pdf"

-     pdf_path = pdf_generator.generate_pdf(html_content, pdf_filename)
...
- HTML(string=html_content).write_pdf('test_sinhala.pdf')

- print("✓ Test PDF generated: test_sinhala.pdf")
...
- chart_cache_key = hashlib.md5(str(data).encode()).hexdigest()

- if not chart_exists(chart_cache_key):
